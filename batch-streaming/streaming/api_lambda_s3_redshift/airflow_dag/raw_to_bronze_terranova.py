import os
import ast
import logging

from airflow.operators.trigger_dagrun import TriggerDagRunOperator
from datetime import datetime, timedelta
from airflow import DAG
from airflow.models import Variable
from airflow.operators.dummy import DummyOperator
from airflow.providers.amazon.aws.operators.redshift_sql import RedshiftSQLOperator
from airflow.utils.task_group import TaskGroup
from airflow.operators.trigger_dagrun import TriggerDagRunOperator
from airflow.sensors.external_task_sensor import ExternalTaskSensor
from common.config.owners import Owners

department = "terranova"
str_tables_list = Variable.get(f"{department}_bronze_tables")
tables_list = ast.literal_eval(str_tables_list)

def create_taskgroup(task_group_prefix, table, department):
    with TaskGroup(group_id=f"{task_group_prefix}_{table}") as tg1:
        
        try:
            create_table = _create_table(
                table,
                f"queries/bronze/{department}/{table}/ddl_bronze_{table}.sql",
            )
        except Exception as e:
            logging.error(f"Erro ao criar a tabela {table}: {e}")
            raise  # Rethrow exception to fail the DAG properly

        try:
            populate_table = _populate_table(
                table,
                f"queries/bronze/{department}/{table}/dml_bronze_{table}.sql",
            )
        except Exception as e:
            logging.error(f"Erro ao popular a tabela {table}: {e}")
            raise  # Rethrow exception to fail the DAG properly

        # Define dependency between tasks
        create_table >> populate_table

        return tg1

def _create_table(table, sql_name):

    return RedshiftSQLOperator(
        task_id=f"create_query_{table}_job",
        configuration={
            f"{sql_name}"
        },
        location="us-east1",
        redshift_conn_id="aws_redshift_conn",
    )

def _populate_table(table, sql_name):

    return RedshiftSQLOperator(
        task_id=f"populate_query_{table}_job",
        configuration={
            f"{sql_name}"
        },
        location="us-east1",
        redshift_conn_id="aws_redshift_conn",
    )

default_args = {
    "owner": Owners.DATA_ENGINEERING.value,
    "depends_on_past": False,
    "email": [""],
    "email_on_failure": False,
    "email_on_retry": False,
    "retries": 1,
    "retry_delay": timedelta(minutes=5),
    "max_active_runs": 1,
}

with DAG(
    "raw_to_bronze_terranova",
    default_args=default_args,
    description="",
    schedule_interval=None,
    start_date=datetime(2024, 1, 10),
    
    catchup=False
) as dag:

    start = DummyOperator(task_id="start")
    end = DummyOperator(task_id="end")

    # Tarefa para disparar a próxima DAG após a conclusão
    trigger_next_dag = TriggerDagRunOperator(
        task_id="trigger_bronze_to_silver",
        trigger_dag_id="bronze_to_silver_terranova",
        execution_date="{{ execution_date }}",
        wait_for_completion=False,
    )

    for table_info in tables_list:
        table = table_info["table"]
        tg_group = create_taskgroup("carrega_bronze", table, department)
        start >> tg_group
    
    tg_group >> trigger_next_dag >> end
