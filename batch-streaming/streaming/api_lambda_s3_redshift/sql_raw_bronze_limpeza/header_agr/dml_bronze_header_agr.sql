INSERT INTO `bronze_terranova.header_agr` (
    id,
    active,
    created_at,
    updated_at,
...
SELECT
    ...
    DATE(TIMESTAMP(REGEXP_EXTRACT(data, r'"created_at":\s*"([^"]+)"'))) AS created_at, -- Apenas a data
    DATE(TIMESTAMP(REGEXP_EXTRACT(data, r'"updated_at":\s*"([^"]+)"'))) AS updated_at, -- Apenas a data
...
WHERE
    REGEXP_CONTAINS(data, r'rota/header_agr:');
