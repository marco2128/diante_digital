...
SELECT
    ...
    REGEXP_REPLACE(
        JSON_EXTRACT_SCALAR(REGEXP_REPLACE(data, r'^.*rota/questions:\s*', ''), '$.aspect.aspecto'),
        r'\\u00eancia',
        'ência'
    ) AS aspecto,
...
SELECT
    ...
    REGEXP_REPLACE(
        JSON_EXTRACT_SCALAR(REGEXP_REPLACE(data, r'^.*rota/questions:\s*', ''), '$.route.description'),
        r'\\u00eancia',
        'ência'
    ) AS description,
    ...
    REGEXP_REPLACE(
        REGEXP_REPLACE(
            REGEXP_REPLACE(
                REGEXP_REPLACE(
                    REGEXP_REPLACE(
                        JSON_EXTRACT_SCALAR(REGEXP_REPLACE(data, r'^.*rota/questions:\s*', ''), '$.checklist.checklist'),
                        r'\\u00e9',
                        'é'
                    ),
                    r'\\u00e7',
...
SELECT
    ...
    -- Correção de caracteres Unicode no campo criticidade
    REGEXP_REPLACE(
        REGEXP_REPLACE(
            JSON_EXTRACT_SCALAR(REGEXP_REPLACE(data, r'^.*rota/questions:\s*', ''), '$.criticality.criticidade'),
            r'\\u00e9',
            'é'
        ),
        r'\\u00ed',
...
SELECT
    ...
    REGEXP_REPLACE(
        JSON_EXTRACT_SCALAR(REGEXP_REPLACE(data, r'^.*rota/questions:\s*', ''), '$.pilar.pilar'),
        r'\\u00e7a',
        'ça'
    ) AS pilar,
...
SELECT
    ...
    REGEXP_REPLACE(
        JSON_EXTRACT_SCALAR(unit, '$.unidade'),
        r'\\u00e1gua',
        'água'
    ) AS unidade,
