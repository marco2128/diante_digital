INSERT INTO `bronze_terranova.gooddaytask` (
    id, name, description, initial_date, final_date, reprogram_date,
    planned, accomplished, comment, action, created_at, updated_at, responsible, status, timestamp
)
SELECT 
  ...
  SAFE_CAST(REGEXP_EXTRACT(data, r'"planned":\s*"([^"]*)"') AS NUMERIC) AS planned,
  SAFE_CAST(REGEXP_EXTRACT(data, r'"accomplished":\s*"([^"]*)"') AS NUMERIC) AS accomplished,
...
WHERE 
  REGEXP_CONTAINS(data, r'rota/gododdaytask:');
